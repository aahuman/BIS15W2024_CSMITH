...
```{r}
display.brewer.pal(5,"PuBu") #sequential palette
```
...
```{r}
q+scale_color_brewer(palette = "Purples")+
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
```{r}
q+scale_color_brewer(palette = "PuRd") #make sure to use color_brewer, not fill_brewer!
```
...
```{r}
my_palette <- paletteer_d("tvthemes::spongeBob") #store your palette
```
